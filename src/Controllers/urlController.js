...
export async function EncodeUrl(req, res) {
	try {
		...
		if (urlNotShort) {
			...
			await connection.query(
				`INSERT INTO "shortUrls" ("urlId","userId","shortUrl") 
            VALUES ($1,$2,$3)`,
				[urlId, id, shortUrl]
			);
...
export async function OpenUrl(req, res) {
	try {
		...
		if (existUrl) {
			...
			await connection.query(
				`INSERT INTO visits ("shortUrlId") 
            VALUES ($1)`,
				[shortUrlId]
			);

			const { rows: views } = await connection.query(
				`SELECT COUNT(v."shortUrlId") as viewsNumber FROM visits v
